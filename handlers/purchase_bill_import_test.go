...
func TestHandleParseCSVItemsParsesUploadedCSV(t *testing.T) {
	...
	if items[1].ProductID != nil {
		t.Fatalf("second item product id = %v, want nil (blank column means new item)", *items[1].ProductID)
	}
}
...
func TestParseOptionalProductID(t *testing.T) {
	cases := []struct {
		...
		{"blank means new item", "", nil},
		{"garbage is ignored, not an error", "abc", nil},
		{"zero is ignored", "0", nil},
		{"negative is ignored", "-1", nil},
		{"valid id parses", "42", intPtr(42)},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := parseOptionalProductID(tc.value)
			if (got == nil) != (tc.want == nil) {
				t.Fatalf("parseOptionalProductID(%q) = %v, want %v", tc.value, got, tc.want)
			}
			if got != nil && tc.want != nil && *got != *tc.want {
				t.Fatalf("parseOptionalProductID(%q) = %d, want %d", tc.value, *got, *tc.want)
			}
		})
	}
}
