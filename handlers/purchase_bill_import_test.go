...
func TestHandleDownloadPurchaseBillTemplateReturnsExcelWorkbook(t *testing.T) {
	...
	if got := resp.Header.Get("Content-Type"); !strings.Contains(got, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
		t.Fatalf("content type = %q, want xlsx content type", got)
	}
	if got := resp.Header.Get("Content-Disposition"); !strings.Contains(got, "purchase-bill-template.xlsx") {
		t.Fatalf("content disposition = %q, want .xlsx filename", got)
	}
